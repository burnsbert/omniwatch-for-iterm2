...
export function createRow(handlers) {
  ...
  const el = h('div', {
    ...
    h('div', { class: 'ow-row-main' }, [
      h('div', { class: 'ow-row-line1' }, [pathEl, age]),
      h('div', { class: 'ow-row-line2' }, [chip, tab, pill, name, dot, muteEl]),
    ]),
  ]);
  el.__ow = { stateEl, head, tail, age, chip, tab, pill, name, dot, muteEl };
  wireRow(el, handlers);
  return el;
}
...
/** List-view table row: state · agent · tab · path · name/label · age · color. */
export function createTableRow(handlers) {
  ...
  el.__ow = { stateEl, stateText, chip, tab, head, tail, pill, name, age, dot, dotName };
  wireRow(el, handlers);
  return el;
}
