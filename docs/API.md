...
| `GET /api/v1/diagnostics` | `{"python":{"path":"/usr/bin/python3","version":"3.9.6"},"iterm":{"status":"ok","error":""},"automation":"ok"\|"denied"\|"unknown","tab_colors":{"package":true,"package_source":"vendor"\|"site-packages"\|null,"reachable":true\|false\|null},"claude_credentials":true\|false\|null,"codex_credentials":true\|false\|null,"config_dir":"…","log_path":"…","demo":false}`. `null` means unknown. `tab_colors.package_source` says where the `iterm2` package was found: Omniwatch's own vendor dir (installed by `install.sh` by default) or some interpreter's real site-packages; `null` if `package` is `false`. The Claude credential check may run `security` (Keychain). `automation` is derived from `iterm.status`: `ok`→`ok`, `not_authorized`→`denied`, anything else→`unknown` |
